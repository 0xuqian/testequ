  ...
  const myPrice = useMemo(() => {
    if (noLiquidity) {
      ...
      if (currencyAAmount && currencyBAmount) {
        return currencyAAmount.divide(currencyBAmount).divide(divide).multiply(multiply).toSignificant(6)
      }
    }
    return undefined
...
export function useZapIn({
  ...
  const _dependentAmount = useMemo(() => {
    ...
    return undefined
  }, [canZap, chainId, currencyA, currencyB, dependentField, independentAmount, pair, divide, multiply])
